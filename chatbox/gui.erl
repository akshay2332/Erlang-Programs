  ...
  receive
    ...
    {result, ClientPID, Ref, Response} ->
      ...
      case Response of
        ...
        err_same ->
          write_chatroom(with_label(St#state.client, ?SYSTEM), "+ Error: Could not change nickname. Requested nickname same as current nickname!");
        %% nickname is used by someone else
        err_nick_used ->
          write_chatroom(with_label(St#state.client, ?SYSTEM), "+ Error: Could not change nickname. Requested nickname already used by another client!");
        %% we successfully changed nickname
        ok_nick ->
          write_chatroom(with_label(St#state.client, ?SYSTEM), "+ Nickname updated! New nickname: \"" ++ Nick ++ "\".")
      end
  end.
